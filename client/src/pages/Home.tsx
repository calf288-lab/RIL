...
const propertyPreview: Array<{ title: string; city: string; location: string; price: string; priceValue: number; meta: string; badge: string; description: string; mortgageAvailable: boolean; photoUrl?: string }> = [
  ...
  { title: "Апартаменты у Кремля", city: "Казань", location: "Центр", price: "11,2 млн ₽", priceValue: 11.2, meta: "1-комн. · 42 м²", badge: "В центре", description: "Компактный городской формат рядом с набережной и историческим центром.", mortgageAvailable: false, photoUrl: "/photos/fasad-tsentr.jpg" },
  { title: "Семейный квартал «Мой город»", city: "Набережные Челны", location: "Новый город", price: "5,9 млн ₽", priceValue: 5.9, meta: "3-комн. · 76 м²", badge: "Для семьи", description: "Просторная планировка, закрытая территория и места для хранения.", mortgageAvailable: true, photoUrl: "/photos/kvartal-vid-sverhu.jpg" },
  { title: "Дом у Камы", city: "Альметьевск", location: "Центральный район", price: "6,7 млн ₽", priceValue: 6.7, meta: "2-комн. · 61 м²", badge: "Под ипотеку", description: "Светлая квартира с отделкой и быстрым выходом на сделку.", mortgageAvailable: true, photoUrl: "/photos/gostinaya-otdelka.jpg" },
...
export default function Home() {
  ...
  return <main className="galactic-shell">
    <header className={`galactic-nav ${scrolled ? "galactic-nav-scrolled" : ""}`}>
      ...
      {mobileOpen && <div className="galactic-mobile-nav"><a href="#agent" onClick={() => setMobileOpen(false)}>Как работает</a><a href="#properties" onClick={() => setMobileOpen(false)}>Каталог</a><a href="#benefits" onClick={() => setMobileOpen(false)}>Возможности</a><a href="#trust" onClick={() => setMobileOpen(false)}>Об Амире</a><a href="#faq" onClick={() => setMobileOpen(false)}>Вопросы</a><a href="#contact" onClick={() => setMobileOpen(false)}>Контакты</a><a href="tel:+79274099179">📞 +7 927 409-91-79</a></div>}
    </header>
    ...
    <section id="agent" className="agent-intro section-dark">
      <div className="section-inner agent-intro-grid"><div className="agent-intro-copy"><span className="section-eyebrow">ПЛАВАЮЩИЙ АГЕНТ НА САЙТЕ</span><h2>Помогает посетителю<br /><span className="text-gradient-blue">не потеряться в выборе</span></h2><p>Вместо навязчивого звонка — спокойный диалог в удобный момент. Амир задаёт несколько вопросов, отвечает по делу и предлагает контакт только когда это действительно полезно.</p><button className="inline-link" onClick={() => { trackGoal("agent_intro_open"); }}>Открыть агента внизу экрана <ArrowRight size={16} /></button></div><div className="agent-preview-card"><div className="preview-glow" /><div className="preview-head"><div className="agent-avatar"><Bot size={21} /></div><div><b>Амир · ИИ-агент</b><small><span /> Сейчас онлайн</small></div><span className="preview-dots">•••</span></div><div className="preview-body"><div className="agent-message agent-message-ai">Приветствую! Ищете квартиру для проживания или инвестиций в Казани?</div><div className="preview-quick"><span>Для проживания</span><span>Для инвестиций</span></div><div className="agent-message agent-message-user">Для проживания. Рассматриваю центр.</div><div className="agent-message agent-message-ai">Понял. Нужна ли помощь с одобрением ипотеки?</div></div><div className="preview-footer"><MessageCircle size={13} /> Диалог продолжается без перезагрузки страницы</div></div></div>
    </section>
